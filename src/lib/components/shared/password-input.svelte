<script lang="ts">
    ...
    import { ExclamationCircle, Eye, EyeOff, Icon } from "svelte-hero-icons";

    import { validate } from "$actions/form";
    import { classNames } from "$utils/style";

    import Hint from "$components/shared/hint-text.svelte";
...
<div class={classNames("relative mt-1", classes)}>
    ...
    <input
        ...
        {...$$restProps}
        class={classNames(
            "peer block w-full appearance-none rounded-md border px-3 py-2 placeholder-gray-400 shadow-sm focus:enabled:outline-none focus:enabled:ring-1 sm:text-sm disabled:bg-gray-100",
            $$slots.prefix && "pl-10",
            $$slots.suffix && "pr-10",
            errors?.length
                ? "pr-10 border-red-300 text-red-900 focus:enabled:border-red-500 focus:enabled:ring-red-500"
    ...
    <button
        type="button"
        class={classNames(
            "absolute inset-y-0 z-10 flex items-center px-1.5 mr-1.5 transition-all duration-75 ease-in-out",
    ...
    {#if errors?.length}
        <div
            class={classNames(
                "absolute inset-y-0 right-0 z-10 flex items-center mr-1.5",
        ...
        >
            <Popover class="relative flex h-full items-center">
                ...
                >
                    ...
                    <span
                        class="absolute left-1/2 -bottom-2 z-10 -translate-x-1/2 border-x-8 border-t-8 border-x-transparent border-t-red-100 drop-shadow-sm"
                    />
                </Transition>
            </Popover>
        </div>
